...
Dispatch lets developers manage Claude Code instances on their local machine by calling a phone number or typing commands in a hosted dashboard. You talk to an AI orchestrator, issue coding requests, and your local machine executes them — no keyboard required. A companion process bridges to local coding-agent CLIs (Cursor, Claude) and streams output back to the web app. A web dashboard handles project management and task history.
...
1. User speaks (via phone call through Twilio) or types a request in Dashboard.
...
4. Local helper (companion) claims the command, executes it in project context, and streams logs.
